...
regressor_rf = joblib.load("/home/anqil/earthcare/data/rf_regressor_ten_days_three_channels_20250429224026.joblib")

regressor_xgb = xgb.Booster()
regressor_xgb.load_model("/home/anqil/earthcare/data/xgb_regressor_ten_days_three_channels.json")

regressor_nn = joblib.load("/home/anqil/earthcare/data/nn_model_ten_days_three_channels.pkl")
print("Model loaded")
...
def repackage_xarray(X_test, y_test, y_pred_dict, xds, height_grid=np.arange(1e3, 15e3, 100)):
    """
    Repackage the test data into xarray for plotting
    """
    if "band" in y_test.dims:
        X_copy_y = y_test.isel(band=0).expand_dims(dim=dict(height=len(height_grid), vars=2))
    else:
    ...
    ds = xr.merge([X, y])
    ds = ds.reindex_like(xds, copy=False).assign_coords(time=xds.time, profileTime=xds.profileTime)
...
for band in ds_org.band:
    ...
    axes[band].plot([min_val, max_val], [min_val, max_val], "r--", label="True = Predicted")
...
axes[1].set_xticklabels(np.interp(axes[1].get_xticks(), mdates.date2num(ds["time"]), ds.latitude).round(2))

secax = axes[1].secondary_xaxis("bottom")
secax.set_xlabel("Longitude [degrees_east]")
secax.set_xticks(axes[1].get_xticks())
secax.set_xticklabels(np.interp(axes[1].get_xticks(), mdates.date2num(ds["time"]), ds.longitude).round(2))
...
for i, y_pred in enumerate(ds[y_pred_dict.keys()].dropna(dim="nray").values()):
    ...
    h = np.histogram2d(ds.dropna(dim="nray").y_true, y_pred, bins=(30, 30), density=True)
    axes[i].contour(h[1][:-1], h[2][:-1], h[0].T)

    # Add text with MSE and R2
    axes[i].text(0.1, 0.9, f"RMSE: {np.sqrt(mean_squared_error(ds.dropna(dim='nray').y_true, y_pred)):.2f}", transform=axes[i].transAxes)
    axes[i].text(0.1, 0.85, f"R2: {r2_score(ds.dropna(dim='nray').y_true, y_pred):.2f}", transform=axes[i].transAxes)

    # Add diagonal line
    min_val = min(ds.dropna(dim="nray").y_true.min(), y_pred.min())
    max_val = max(ds.dropna(dim="nray").y_true.max(), y_pred.max())
    axes[i].plot([min_val, max_val], [min_val, max_val], "r--", label="True = Predicted")
